...
export function CarouselCard({ card, index, onClick }: CarouselCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 z-10 p-3 pt-4 sm:p-6 sm:pt-8">
        <p className="text-[10px] sm:text-[11px] text-white/60 uppercase tracking-widest font-medium mb-1 sm:mb-2">
          {card.category}
        </p>
        <h3 className="text-base sm:text-2xl md:text-3xl font-semibold text-white leading-tight">
          {card.title}
        </h3>
      </div>

      {/* Boton "+" - 44px touch target on all sizes */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          onClick?.();
        ...
        className={cn(
          "absolute bottom-2 right-2 sm:bottom-4 sm:right-4 z-20",
