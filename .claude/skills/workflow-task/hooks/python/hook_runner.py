...
def run_hook(hook_type: str, args: list[str]) -> int:
    ...
    try:
        ...
        logger = get_logger(hook_type)
        logger.hook_start(context)
    except ImportError:
        logger = None
    ...
    try:
        ...
        if logger:
            logger.hook_end(success=(result == 0))
        return result
