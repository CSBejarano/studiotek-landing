...
class AgentDecision:
    ...
    Supports both minimal format (id, scenario, decision) and extended format
    with additional fields from existing YAML files (context, confidence_score,
    validated_count, failed_count, last_used, tags).
    """
    id: str
    scenario: str
    decision: str
    ...
    # Extended fields for backwards compatibility with existing YAML files
    validated_count: int = 0
    failed_count: int = 0
    last_used: Optional[str] = None
    tags: List[str] = field(default_factory=list)
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary, including extended fields if present."""
        result = {
            "id": self.id,
        ...
        # Include extended fields if they have non-default values
        if self.validated_count != 0:
            result["validated_count"] = self.validated_count
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AgentDecision":
        """
        Create from dictionary. Backwards-compatible with both minimal and
        extended YAML formats.

        Handles field name variations:
        - 'scenario' or 'context' for the scenario field
        - 'confidence' or 'confidence_score' for confidence
        """
        # Support both 'scenario' and 'context' field names
        scenario = data.get("scenario") or data.get("context", "")

        # Support both 'confidence' and 'confidence_score' field names
        confidence = data.get("confidence") or data.get("confidence_score", 0.8)
...
class AgentBlocker:
    ...
    Supports both minimal format (id, symptom, solution) and extended format
    with additional fields from existing YAML files (description, root_cause,
    prevention, severity, discovered, resolved, occurrences, tags).
    """
    id: str
    symptom: str
    solution: str
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.discovered:
            result["discovered"] = self.discovered
        if self.resolved:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AgentBlocker":
        ...
        Handles severity as string (case-insensitive) and normalizes to lowercase.
        """
