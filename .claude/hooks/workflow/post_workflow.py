...
def setup_logging(project_root: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("post_workflow")
...
def handle_post_workflow(context: dict[str, Any], args: list[str]) -> int:
    ...
    logger.info("=" * 50)
    logger.info("Post-workflow hook triggered")
    ...
    try:
        ...
        logger.info(f"Memory manager loaded for project: {project_root}")

        # Check if session exists
        session_file = project_root / "ai_docs" / "memoria" / "short_term.json"
        if not session_file.exists():
            print("[hooks] No active session to close")
            return 0
        ...
        if not manager.short_term:
            print("[hooks] Could not load session")
            return 0
        ...
        print(f"[hooks] Error in post_workflow: {e}", file=sys.stderr)
...
def main() -> int:
    ...
    try:
        ...
        print(f"[post_workflow] Warning: Invalid JSON input: {e}", file=sys.stderr)
        context = {}
