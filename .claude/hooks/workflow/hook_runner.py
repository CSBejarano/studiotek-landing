#!/usr/bin/env python3
"""
Hook Runner - Main orchestrator for workflow hooks v2.0.

This module routes hook events to their appropriate handlers and manages
the project root discovery and error handling.
...
def get_hooks_root() -> Path:
    """
    Discover the hooks root directory.

    Returns the directory containing the workflow hooks (core/, models/, etc.).
    """
...
def get_project_root() -> Path:
    ...
    if project_root := os.environ.get("CLAUDE_PROJECT_DIR"):
        return Path(project_root)

    if project_root := os.environ.get("CLAUDE_PROJECT_ROOT"):
        return Path(project_root)

    # Walk up from hooks root to find project root (.claude/hooks/workflow -> project)
    hooks_root = get_hooks_root()
    return hooks_root.parent.parent.parent
...
def run_hook(hook_type: str, args: list[str]) -> int:
    ...
    hooks_root = get_hooks_root()
    core_dir = hooks_root / "core"
    models_dir = hooks_root / "models"

    for module_dir in [core_dir, models_dir, hooks_root]:
    ...
    try:
        if hook_type == "pre_workflow":
            ...
            print(f"[hooks] Unknown hook type: {hook_type}", file=sys.stderr)
            return 1

    except ImportError as e:
        print(f"[hooks] Failed to import handler for {hook_type}: {e}", file=sys.stderr)
        return 1
    except Exception as e:
        print(f"[hooks] Error in {hook_type} hook: {e}", file=sys.stderr)
